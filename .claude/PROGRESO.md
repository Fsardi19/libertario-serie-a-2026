# Progreso - SERIE A 2026 Libertario

**Ultima actualizacion:** 2026-01-30 (Sesion 6 - Due Diligence Legal)
**Estado General:** EN CURSO

## Objetivo Final
Informe ejecutivo para Junta Directiva sobre Serie A 2026 - ENTREGADO
Seguimiento continuo de datos financieros y operativos

---

## Datos Financieros Actualizados (Comite Directivo 30-01-2026)

### Ventas 2025 - CIERRE REAL
| Metrica | Valor |
|---------|-------|
| **Ventas Real** | **$7.22M** (101% del plan) |
| Ventas Deck | $7.17M |
| Tiendas cierre | 18 |

### Proyecciones 2026
| Metrica | 2025 Real | 2026 Presupuesto | Variacion |
|---------|-----------|------------------|-----------|
| **Ventas** | $7.2M | **$16.7M** | +132% |
| Retail | $6.3M | $15.0M | +138% |
| Wholesale | $0.9M | $1.7M | +89% |
| **EBITDA** | -$0.8M | **+$0.7M** | Positivo |
| EBITDA Operacion | $0.79M | $2.68M | +239% |
| G&A | -$1.65M | -$1.92M | +16% |
| Tiendas | 18 | 38 | +111% |

### Break-even
- **Fecha proyectada:** Julio 2026
- **Modelo validado:** Bogota Calle 79 - break-even mes 1 (vs presupuesto mes 3)

---

## Tiendas por Pais (Enero 2026)

| Pais | 2025 | 2026 Plan | Operativas Ene-26 | En construccion |
|------|------|-----------|-------------------|-----------------|
| Colombia | 10 | 16 | 10 | 3 |
| Mexico | 2 | 6 | 2 | 1 |
| Costa Rica | 1 | 2 | 1 | 1 |
| India | 1 | 6 | 1 | 1 |
| Chile | 0 | 4 | 1 | 0 |
| Estados Unidos | 4 | 4 | 4 | 0 |
| **Total** | **18** | **38** | **19** | **6** |

---

## Necesidad de Capital

| Componente | Monto | Estado |
|------------|-------|--------|
| Ronda actual | $1.9M | Asegurado |
| Socios minoritarios | $0.3M | Asegurado |
| **Gap a cubrir** | **$1.4M** | En proceso |
| **Total necesidad** | **$3.3M** | |

### Alternativas Exploradas para Gap ($1.4M)
| Fuente | Monto |
|--------|-------|
| Mifel Mexico (bancario local) | $0.40M |
| India (prestamos entrepreneurs) | $0.25M |
| Colombia (leasing sin firmas solidarias) | $0.25M |
| Extension ronda | $0.50M |
| **Total** | **$1.40M** |

---

## Que ha Cambiado vs Plan

| Dimension | Estado | Comentario |
|-----------|--------|------------|
| Ventas por tienda | OK | En linea con el plan |
| Margen operativo | OK | En linea con el plan |
| Maduracion tiendas | MIXTO | Colombia/India OK, Mexico/Costa Rica rezagados |
| Ritmo de aperturas | RETRASADO | Capital recibido tarde |
| Flujo de caja | PRESIONADO | Alquileres, locales vacios |

**Causas principales:**
1. Capital recibido mas tarde de lo esperado
2. Mayor gasto operativo por alquileres y locales vacios
3. Desplazamiento del punto de equilibrio a Julio 2026

---

## KPIs Modelo Optimizado (Bogota Calle 79)

| KPI | Dic 2025 |
|-----|----------|
| Ticket promedio | $12 |
| Transacciones/dia | 79 |
| Ventas/dia | $951 |
| Margen bruto | 66.02% |
| EBITDA tienda | $369 |
| Break-even real | Mes 1 |
| Break-even presupuesto | Mes 3 |

---

## Timeline del Proyecto

| Hito | Fecha | Estado |
|------|-------|--------|
| Proceso Serie A activo | 2026 | En curso |
| Break-even consolidado | Julio 2026 | Proyectado |
| Cierre Serie A | Q1 2027 | Pendiente |
| Base valoracion | Revenue 2026: $16.7M | Definido |

---

## Entregables Finales

| Archivo | Descripcion | Estado |
|---------|-------------|--------|
| INFORME_JUNTA_SERIE_A_2026.pdf | Presentacion PDF profesional (12 slides) | ENTREGADO |
| INFORME_JUNTA_SERIE_A_2026_v2.html | Version HTML interactiva | ENTREGADO |
| INFORME_JUNTA_SERIE_A_2026.md | Version Markdown | ENTREGADO |
| datos_informe_junta.json | Datos estructurados para trazabilidad | ENTREGADO |
| DASHBOARD_EMBUDO_INVERSIONISTAS.html | Dashboard interactivo embudo | ENTREGADO |
| EMAIL_JUNTA_SERIE_A.md | Email para compartir con Junta | ENTREGADO |

---

## Negociacion Roman Irrure

### Propuesta Aprobada por Junta (30-01-2026)
| Componente | Terminos |
|------------|----------|
| Inversion inicial | $125,000 en nota existente |
| Terminos nota | $40M cap, 20% descuento, 6%, 24 meses |
| Contribucion in-kind | Hasta $125,000 adicionales via KPIs |
| **Inversion total potencial** | **$250,000** |
| Rol en Junta | Puerta abierta para futuro, sin compromiso actual |

### Beneficios Comunicados a Roman
- Cap $40M (vs nueva nota que abrira a $50M)
- Ticket flexible aceptado ($125K)
- KPIs permiten duplicar participacion

### Estado Actual
- **Propuesta formal enviada** (PROPUESTA_ROMAN_IRRURE.docx)
- Pendiente: Respuesta de Roman
- Si acepta: Definir KPIs especificos

---

## Propuesta ONEtoONE

| Item | Detalle |
|------|---------|
| Contacto | Juan Emilio Posada, Partner |
| Mandato | $15M Serie A + estrategia India + exit advisory |
| Estado | Pendiente decision de Junta |

---

## Analisis de Multiplos

### Comparables (9 empresas)
| Empresa | Pais | Valoracion | Multiplo |
|---------|------|------------|----------|
| La Colombe | USA | $900M | ~4.5x |
| Blue Bottle | USA | $700M | ~7x |
| Blank Street | USA/UK | $500M | ~12.5x |
| Blue Tokai | India | $180M | 6.9x |
| Third Wave | India | $155M | 8.9x |
| WatchHouse | UK | $95M | 5.2x |
| GoodNews Coffee | Espana | EUR30-40M | ~10-14x |
| Proud Mary | AUS/USA | $60M | 4.3x |
| Subko | India | $34M | >20x |

### Valoracion Target Libertario
- **Rango recomendado:** 3.5x - 5.0x (conservador)
- **Valoracion:** $56M - $80M pre-money (sobre $16.7M revenue 2026)
- **Diferenciador:** Unica comparable con trayectoria a EBITDA positivo

---

## Decisiones Pendientes para Junta

| # | Tema | Recomendacion |
|---|------|---------------|
| 1 | Roman Irrure - Terminos NED + CLA | Aprobar |
| 2 | Sobre-suscripcion / Extension ronda | Aprobar |
| 3 | ONEtoONE - Solicitar propuesta formal | Decidir |
| 4 | Alternativas financiamiento gap $1.4M | Revisar |

---

## Embudo de Inversionistas (Actualizado 30-01-2026)

| Categoria | Monto | Inversionistas |
|-----------|-------|----------------|
| Cerrado / Comprometido | $4.20M | 14 |
| Pipeline Activo | $1.38M | 6 |
| **Total vs Meta $5M** | **$5.58M (112%)** | 20 |
| Leads Serie A | - | 3 fondos |
| Interes Futuro | $4.10M | 14 |
| Cierre Perdido | $3.25M | 13 |
| Total Contactados | - | 50 |

---

## Nota Convertible - Analisis de Extension

### Terminos Actuales (Nota $5M)
| Termino | Valor |
|---------|-------|
| Valuation Cap | $40,000,000 (pre-money) |
| Descuento | 20% |
| Interes | 6% anual |
| Plazo | 24 meses |
| Valoracion efectiva | $32M ($40M x 0.80) |
| Post-money | $37M |
| % para holders | 13.51% |

### Analisis Extension $500K (Nuevo Inversionista)

| Escenario | % Original $5M | Efecto |
|-----------|----------------|--------|
| Sin extension | 13.51% | Base |
| Extension mismos terminos | 13.33% | -0.18pp dilucion |
| Extension cap $50M (recomendado) | 13.51% | Sin dilucion |

### Recomendacion
- **Emitir nota separada** con cap $50M para los $500K
- No modifica documentos existentes
- Protege completamente a inversionistas originales
- Justificacion: Menor riesgo para inversionista posterior

### Documentos Relacionados
- Template original: `output/datos/CONVERTIBLE_LOAN_AGREEMENT_TEMPLATE.md`
- Email abogada: `output/EMAIL_ANA_CRISTINA_EXTENSION_NOTA.md`

---

## Due Diligence Legal (Actualizado 02-02-2026)

### Estructura Corporativa Completa (6 Paises)

```
LIBERTARIO HOLDING (Panama)
    └── LIBERTARIO COFFEE ROASTERS CORP (Panama) - LCR Corp
            │
            ├── Futura Coffee Roasters LLC (USA) ───────── 100%
            ├── Libertario Coffee Roasters S.A.S (Colombia) ─ 100%
            ├── Libertario Coffee Roasters SA de CV (Mexico) ─ 75% + Masanamacaya 25%
            ├── Libertario Coffee Roasters SpA (Chile) ───── 92.5% + SAMA SpA 7.5%
            ├── Libertario Coffee Roasters S.A. (Costa Rica) ─ 74% + Inv. Oshala 16-18%
            └── Libertario Coffee Roasters PVT. LTD (India) ─ 55% + Miguel Villaquiran 45%
```

### Cap Table LCR Corp (Panama) - Holding Principal

| Accionista | % |
|------------|---|
| Arlen Development Inc | 52.32% |
| Cassland Group S.A. | 40.02% |
| Bagua Holdings | 2.79% |
| SI & B LLC (Juan Carlos Sardi) | 1.90% |
| Miguel Angel Ladino | 1.48% |
| Sebastian Villamizar | 1.48% |
| **TOTAL** | **100%** |

### Cap Table Arlen Development (Panama)

| Accionista | % |
|------------|---|
| Coffee Hub S.A.S | 54.10% |
| Inversiones Crono S.A.S | 34.40% |
| Sebastian Villamizar | 5.30% |
| Felipe Pinzon | 3.00% |
| Laura Escobar | 1.60% |
| Camilo Reyes | 1.60% |
| **TOTAL** | **100%** |

### Accionistas Individuales - Beneficial Ownership en LCR

| Persona | % en LCR | Via |
|---------|----------|-----|
| Felipe Sardi (BP) | 38.02% | IC→AD, CH→AD, CG directo |
| Orlando Sardi | ~15.44% | Via CG 38.58% x 40.02% |
| Sebastian Villamizar | ~8.26% | Directo 1.48% + AD 5.30% + CH→AD |
| Stella Aristizabal | ~7.66% | Via CG 19.14% x 40.02% |
| Daniel Sardi (Bagua) | 2.79% | Via Bagua Holdings |
| Juan Carlos Sardi (SI&B) | 1.90% | Via SI & B LLC |
| Felipe Pinzon | ~1.57% | Via Arlen 3.00% x 52.32% |
| Miguel Angel Ladino | 1.48% | Directo en LCR Corp |
| Laura Escobar | ~0.84% | Via Arlen 1.60% x 52.32% |
| Camilo Reyes | ~0.84% | Via Arlen 1.60% x 52.32% |

### Estructura por Pais (Corregida)

| Pais | Entidad | % LCR Corp | Socio Local | Mayoria Calif. |
|------|---------|------------|-------------|----------------|
| Panama | LCR Corp (HOLDING) | - | Ver cap table arriba | 70% |
| **USA** | **Futura LLC** | **100%** | **Ninguno** | N/A |
| **Colombia** | **LCR S.A.S.** | **100%** | **Ninguno** | Ley Colombia |
| **Mexico** | **LCR SA de CV** | **75%** | **Masanamacaya 25%** | 76% |
| **Chile** | **LCR SpA** | **92.5%** | **SAMA SpA 7.5%** | 86% |
| **Costa Rica** | **LCR S.A.** | **74%** | **Inv. Oshala 16-18%** | 70% |
| **India** | **LCR PVT. LTD** | **55%** | **Miguel Villaquiran 45%** | 70% |

**Notas:**
- Colombia 100% confirmado por Ana Maria Pelaez (30-01-2026) - NO EXISTE SHA
- India SHA analizado 02-02-2026: Lock-up 5 años, Drag Floor $20M
- Chile SHA analizado 02-02-2026: Lock-up hasta dic 2027, vesting adicional 7.5%

### Clausulas Serie A Clave
- **Preemptive rights:** 7-10 dias segun pais
- **Adhesion:** Nuevos inversionistas firman Adherence Letter al SHA
- **Tag-along:** 50%+1 en todos los paises

### Documentos Analizados
- [x] EP 14289 Panama (migracion BVI->Panama)
- [x] SHA Costa Rica (Oshala)
- [x] SHA Mexico (Masanamacaya)
- [x] Operating Agreement USA (Futura LLC)
- [x] MOA/AOA India
- [x] **SHA India (Miguel Villaquirán) - Mayo 2025** ← NUEVO 02-02-2026
- [x] Bylaws Colombia
- [x] Certificado Existencia Colombia

### Documentos Analizados - SHA Completos
- [x] ~~SHA Colombia~~ - NO EXISTE (confirmado Ana Maria Pelaez 30-01-2026)
- [x] SHA India (Miguel Villaquiran) - ANALIZADO 02-02-2026 (55%/45%, Lock-up 5 años)
- [x] SHA Chile (Santiago Botero/SAMA SpA) - ANALIZADO 02-02-2026 (92.5%/7.5%)

### Documentos Pendientes para Data Room

**Cap Tables Detallados (6 paises):**
- [ ] Panama - Libertario Holding (accionistas)
- [ ] Panama - LCR Corp (VERIFICAR VIGENCIA - tenemos datos)
- [ ] USA - Futura LLC (verificar 100%)
- [ ] Colombia - LCR S.A.S. (verificar 100%, pendiente aporte en especie mid-Feb)
- [ ] Mexico - LCR SA de CV (detalle Masanamacaya)
- [ ] Chile - LCR SpA (detalle SAMA SpA)
- [ ] Costa Rica - LCR S.A. (detalle Oshala)
- [ ] India - LCR PVT. LTD (detalle Miguel Villaquiran)

**Actas de Asamblea Holding 2024-2025:**
- [ ] Libertario Holding (Panama) - Asambleas 2024, 2025
- [ ] LCR Corp (Panama) - Asambleas 2024, 2025
- [ ] Arlen Development (Panama) - Asambleas 2024, 2025

**Certificados de Existencia Actualizados:**
- [ ] Panama - Libertario Holding (Registro Publico)
- [ ] Panama - LCR Corp (Registro Publico)
- [ ] Panama - Arlen Development (Registro Publico)
- [ ] USA - Futura LLC (State of Delaware?)
- [ ] Colombia - LCR S.A.S. (CCB)
- [ ] Mexico - LCR SA de CV (SAT/Registro)
- [ ] Chile - LCR SpA (Conservador)
- [ ] Costa Rica - LCR S.A. (Registro Nacional)
- [ ] India - LCR PVT. LTD (ROC)

### Archivos Generados en output/legal/
1. `estructura_corporativa.json` - Datos estructurados
2. `resumen_legal_serie_a.md` - Analisis consolidado
3. `analisis_corporativo_progreso.md` - Detalle por pais
4. `DOCUMENTOS_PENDIENTES_LEGAL.md` - Lista para equipo legal
5. `inventario_data_room.md` - Archivos disponibles
6. `archivos_pendientes.txt` - PDFs escaneados
7. `sha_analisis.md` - SHA Panama

---

## Archivos del Proyecto

```
SERIE A 2026/
├── 20260130 - Comite Directivo.pdf          <- Datos actualizados
├── output/
│   ├── presentacion/
│   │   ├── INFORME_JUNTA_SERIE_A_2026.pdf   <- ENTREGABLE FINAL
│   │   ├── INFORME_JUNTA_SERIE_A_2026_v2.html
│   │   ├── INFORME_JUNTA_SERIE_A_2026_PDF.html
│   │   ├── INFORME_JUNTA_SERIE_A_2026.md
│   │   ├── DASHBOARD_EMBUDO_INVERSIONISTAS.html
│   │   └── DASHBOARD_KPIs_ROMAN.html        <- Dashboard KPIs Roman
│   ├── datos/
│   │   ├── datos_informe_junta.json
│   │   ├── datos_comite_directivo_20260130.json
│   │   └── CONVERTIBLE_LOAN_AGREEMENT_TEMPLATE.md  <- NUEVO
│   ├── ESCENARIOS_NEGOCIACION_ROMAN.md
│   ├── KPIs_WHOLESALE_ROMAN.md
│   ├── EMAIL_JUNTA_SERIE_A.md
│   ├── EMAIL_ANA_CRISTINA_EXTENSION_NOTA.md
│   ├── EMAIL_ANA_CRISTINA_EXTENSION_NOTA.docx
│   ├── PROPUESTA_ROMAN_IRRURE.docx
│   └── legal/                               <- NUEVO (Due Diligence)
│       ├── estructura_corporativa.json
│       ├── resumen_legal_serie_a.md
│       ├── analisis_corporativo_progreso.md
│       ├── DOCUMENTOS_PENDIENTES_LEGAL.md
│       ├── inventario_data_room.md
│       ├── archivos_pendientes.txt
│       └── sha_analisis.md
└── .claude/
    ├── PROGRESO.md                          <- Este archivo
    └── SESSION_LOG.md
```
